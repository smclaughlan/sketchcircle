...
function InsideSketchbook(props) {
  ...
  const [displayedGoals, setDisplayedGoals] =
    React.useState();
  ...
  const updateDisplayedPosts = () => {
    if (props.posts && props.posts[sketchbookId]) {
      ...
      totalPages = Math.ceil(
        Object.keys(skbPosts).length / postsPerPage
      );
      if (totalPages < 1) {
        totalPages = 1;
      }
      ...
      for (let i = 0; i < postKeys.length; i++) {
        const earliestPostOnPageOrLater =
          i >= pageNum * postsPerPage - postsPerPage;
        const lastPostOnPageOrEarlier =
          i < pageNum * postsPerPage;
        let currPost = skbPosts[postKeys[i]];
        if (
          earliestPostOnPageOrLater &&
          lastPostOnPageOrEarlier
        ) {
  ...
  React.useEffect(() => {
    if (posts && posts.length > 0) {
      let newRefs = Object.keys(posts[sketchbookId]).reduce(
        (acc, value) => {
  ...
  const deletePost = async (postId) => {
    let wasLastPost =
      displayedPosts[displayedPosts.length - 1].id ===
      postId;
    await props.sendDeletePostReq(props.token, postId);
    await props.getPostsReq(sketchbookId);
    let isLastPostDisplayed =
      displayedPosts[displayedPosts.length - 2].displayed;
    if (
      wasLastPost &&
  ...
  let justPosted =
    window.localStorage.getItem("justPosted");
  if (justPosted === "true") {
    justPosted = "false";
  ...
  let justEdited =
    window.localStorage.getItem("justEdited");
  let scrollID = window.localStorage.getItem("scrollID");
  if (
    justEdited === "true" &&
    refs &&
              ...
              <div key={displayedGoals[k].id}>
                <Paper style={{ margin: "20px" }}>
                  <LineGraph
                    ...
                    description={
                      displayedGoals[k].description
                    }
                    owner_id={displayedGoals[k].owner_id}
                    sketchbook_id={
                      displayedGoals[k].sketchbook_id
                    }
                    target={displayedGoals[k].target}
                    targetDate={
                      displayedGoals[k].targetdate
                    }
        ...
        <Container style={{ marginTop: "10px" }}>
          ...
          >
            <form onSubmit={newGoal}>
              ...
              <Typography>
                Create a new goal with a title, description,
                target value to reach, and a target date.
                For example, a title might be "Draw 5
                portrait sketches in 7 days", which would
                have a target value of 5, and a target date
                7 days from now.
              </Typography>
              <div>
                <TextField
      ...
      <Container ref={pageButtons}>
        <Container>
          {displayedPosts.length > 0 ? (
            <Button
              variant="contained"
              href={`/sketchbook/${sketchbookId}/timeline`}
            >
              View Timeline
            </Button>
          ) : (
          ...
          Object.keys(displayedPosts).map((k) => {
            if (displayedPosts[k].displayed === true) {
              return (
                <>
                  <div
                    ref={refs[displayedPosts[k].id]}
                    key={displayedPosts[k].id}
                    style={{ padding: "0px" }}
                  ></div>
                  <Paper style={{ margin: "50px" }}>
                    ...
                    >
                          ...
                          >
                            {displayedPosts[k].avatar ? (
                              <img
                                ...
                                alt={`${displayedPosts[k].username}'s avatar`}
                                src={
                                  displayedPosts[k].avatar
                                }
                              />
                      ...
                      parseInt(props.currentUserId) ? (
                        <>
                          <NavLink
                            to={`/sketchbook/${sketchbookId}/post/${displayedPosts[k].id}/edit`}
                          >
                            <Edit
                              color="primary"
                              onClick={() => {
        ...
        {displayedPosts.length === 0 && props.token ? (
          ...
          >
            No posts found. Introduce yourself or welcome
            the new user!
          </Typography>
        ) : (
          <></>
        ...
        {displayedPosts.length === 0 && !props.token ? (
          ...
          >
            No posts found. Register/sign in to post!
          </Typography>
        ) : (
          <></>
        )}
...
const mapDispatchToProps = (dispatch) => {
  return {
    getPostsReq: (...args) =>
      dispatch(getPostsReq(...args)),
    sendNewGoalReq: (...args) =>
      dispatch(sendNewGoalReq(...args)),
    sendDeletePostReq: (...args) =>
      dispatch(sendDeletePostReq(...args)),
  };
};
